...
def generate_new_latlng_from_distance(cur_lat,
    ...
    new_lng = math.atan2(
        math.sin(bearing_rad) * math.sin(distance /
                                         radius) * math.cos(cur_lat),
...
def generate_new_latlng_vectorized(cur_lat, cur_lng, segment_bearing, distance, radius):
        # Convert inputs to radians
    cur_lat_rad = np.radians(cur_lat)
    cur_lng_rad = np.radians(cur_lng)
    bearing_rad = np.radians(segment_bearing)

    # Compute new coordinates using vectorized operations
    new_lat = np.arcsin(
        np.sin(cur_lat_rad) * np.cos(distance / radius) +
        np.cos(cur_lat_rad) * np.sin(distance / radius) * np.cos(bearing_rad)
    )

    delta_lng = np.arctan2(
        np.sin(bearing_rad) * np.sin(distance / radius) * np.cos(cur_lat_rad),
        np.cos(distance / radius) - np.sin(cur_lat_rad) * np.sin(new_lat)
    )

    new_lng = cur_lng_rad + delta_lng

    # Convert back to degrees
    new_lat_deg = np.degrees(new_lat)
    new_lng_deg = np.degrees(new_lng)
