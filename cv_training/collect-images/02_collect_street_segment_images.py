...
def get_panoid_dict(lat, lng):
    raw_data = streetview.search_panoramas(lat, lng)
    print(type(raw_data[0]), raw_data)
...
def return_optimal_panoid(lat, lng):
    ...
    """
    try:
        panoid_list = streetview.search_panoramas(lat, lng)
    except:
        return []
    ...
    for panoid in panoid_list:
        if panoid.date:
            pano_date = date(int(panoid.date[:4]), int(panoid.date[5:]), 1)
            if PERIOD_SELECTION['max'] >= pano_date >= PERIOD_SELECTION['min']:
                if abs(pano_date - PERIOD_SELECTION['optimal_date']) < current_optimum:
                    current_optimum = abs(
                        pano_date - PERIOD_SELECTION['optimal_date'])
                    optimal_panoid['pano_id'] = panoid.pano_id
        ...
        if panoid.date:
            ...
            if PERIOD_SELECTION['max'] >= pano_date >= PERIOD_SELECTION['min']:
                    period_panoid_list.append(
                        {'pano_id': panoid.pano_id, 'date': pano_date})

    return period_panoid_list

def process_segments(key):
    ...
    if segment_id in restricted_segments:
        ...
        image_log = '{} RestrictedSegment NA NA NA NA NA END'.format(
            segment_id)
    ...
    if len(segment['coordinates']) == 0:
        print('[WARNING] No coordinates for segment {}: {}'.format(
            key, segment['name']))
        #coordinate_unavailable_counter += 1
        image_log = '{} UnavailableCoordinates NA NA NA NA NA END'.format(
            segment_id)
        logger.write(image_log)
        return
    ...
    if segment['coordinates'][0][1] is None or segment['coordinates'][0][2] is None:
        ...
        image_log = '{} UnavailableFirstHeading NA NA NA NA NA END'.format(
            segment_id)
        logger.write(image_log)
        return
...
if SEGMENT_RESTRICTION is not None:
    ...
    unrestricted_segments = set(
        restricted_segment_image_log['segment_id'].unique())
...
Parallel(n_jobs=-1)(delayed(process_segments)(key)
