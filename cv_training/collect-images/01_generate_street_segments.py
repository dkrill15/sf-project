...
from urbanchange_utils import generate_new_latlng_from_distance, generate_new_latlng_vectorized
...
SELECTED_LOCATION = 'SF'
OUTPUT_FILE = 'segment_dictionary_{}.json'.format(SELECTED_LOCATION)
INTERMEDIATE_FILE_PATH = 'intermediate_segment_dictionary_{}.txt'.format(
    SELECTED_LOCATION)
VISUALIZE = False
...
def generate_latlng(linestring, bearing, visualize):
    ...
    for i, (lng, lat) in enumerate(line_segment_coords):
        ...
        while in_bounds:
            new_lat, new_lng = generate_new_latlng_vectorized(
...
temporary_data = AppendLogger(os.path.join(
    OUTPUT_PATH, INTERMEDIATE_FILE_PATH))
...
Parallel(n_jobs=-1)(delayed(process_street)(row) for row in tqdm(range(row_start, len(street_segments))))
...
with open(os.path.join(OUTPUT_PATH, INTERMEDIATE_FILE_PATH), 'r') as file:
    ...
    result_street_segments = file.readlines()
final_line = json.loads(result_street_segments[-1])
last_row = int(list(final_line.keys())[0])

if last_row >= len(result_street_segments) * .75:
    street_segments_dict = {}
    for segment in result_street_segments:
        segment_dict = json.loads(segment)
        for key, item in segment_dict.items():
            street_segments_dict[key] = item
    keys = [int(x) for x in list(street_segments_dict.keys())]
    keys.sort()
    street_segments_dict = {str(i): street_segments_dict[str(i)] for i in keys}
    last_key = int(list(street_segments_dict)[-1])
    if last_key == len(result_street_segments) - 1:
        print(f'[INFO] Genereated coordinates for all {last_key + 1} street segments. Exporting street segment dictionary.')
